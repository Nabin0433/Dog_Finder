...
export default function Search() {
    ...
    const handleQuery = async (query: any) => {
        setLoading(true);
        try {
            const breedsQuery = (query.breeds || [])
                .map((breed: string) => `breeds=${encodeURIComponent(breed)}`)
                .join('&');

            const zipCodesQuery = (query.zipCodes || [])
                .map((zipCode: string) => `zipCodes=${encodeURIComponent(zipCode)}`)
                .join('&');

            const queryString = [zipCodesQuery, breedsQuery]
                .filter(Boolean)
                .join('&');

            const response = await searchDogs(
                `${queryString}&size=${perPage}&from=${(currentPage - 1) * perPage}&sort=${query.sort}&ageMin=${query.ageMin}&ageMax=${query.ageMax}`
            );
